...
import { useProfile, type Profile, type UrgeType } from "@/lib/storage";

export const Route = createFileRoute("/onboarding")({
  component: Onboarding,
});

const GOALS: { label: string; type: UrgeType }[] = [
  { label: "暴食を減らしたい",               type: "food" },
  { label: "間食のお菓子を辞めたい",          type: "snack" },
  { label: "SNSを見る時間を減らしたい",       type: "sns" },
  { label: "夜更かしを終わらせたい",          type: "latenight" },
  { label: "怒りに振り回されないようにしたい", type: "anger" },
  { label: "ネガティブ思考を減らしたい",       type: "negative" },
  { label: "感情の波と上手く付き合いたい",    type: "emotion" },
  { label: "禁煙したい",                     type: "smoke" },
  { label: "お酒との距離を整えたい",          type: "alcohol" },
  { label: "衝動買いを止めたい",             type: "shopping" },
  { label: "ポルノ依存から抜け出したい",      type: "porn" },
  { label: "承認欲求を手放したい",           type: "approval" },
  { label: "先延ばしをやめたい",             type: "procrastination" },
];
...
function Onboarding() {
  ...
  const [selectedGoal, setSelectedGoal] = useState<{ label: string; type: UrgeType } | null>(null);
  ...
  const finalGoalLabel = customGoal.trim() || selectedGoal?.label || "";
  const finalPrimaryType: UrgeType = selectedGoal?.type ?? "other";
      ...
      <div className="mx-auto flex min-h-screen max-w-md flex-col px-6 pt-12 pb-10">
              ...
              {GOALS.map((g) => (
                <button
                  ...
                  className={
                    ...
                    (selectedGoal?.label === g.label && !customGoal
          ...
          {step === 2 && (
            <ul className="space-y-2">
              {[
                ...
                ["脳のクセを、記録で少しずつ可視化", "傾向分析"],
                ["タイプ別に、効く対策を提案", "おまかせ対策"],
                ["「失敗」の概念がない、責めない設計", "セルフコンパッション"],
              ].map(([t, tag]) => (
                <li key={t} className="rounded-2xl border border-border bg-white/5 p-4">
              ...
              {VOICES.map((v) => (
                <button
                  ...
                  className={
                    ...
                    (voice === v.id ? "border-accent/50 bg-accent/10" : "border-border bg-white/5 hover:border-white/20")
                  }
                >
                  <div>
                  ...
                  <div className={"size-2 rounded-full " + (voice === v.id ? "bg-accent shadow-[var(--accent-glow)]" : "bg-white/10")} />
                </button>
              ))}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-accent/20 bg-accent/[0.04] p-4">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  ...
                  <br />感情・人・未来 — 自分の心が動く具体的な言葉を入れてください。
                </p>
              </div>
              <textarea
                value={reason}
              ...
              <div className="space-y-2">
                <p className="px-1 text-[10px] tracking-widest text-muted-foreground">書けないときは、お手本から始めてもいい</p>
                {REASON_EXAMPLES.map((ex, i) => (
                  <button key={i} type="button" onClick={() => setReason(ex)}
        ...
        {step > 0 && step < 5 && (
          <footer className="mt-10 flex items-center justify-between gap-4">
            ...
            <button
              onClick={next}
              disabled={(step === 1 && !finalGoalLabel) || (step === 4 && !reason.trim())}
  ...
  return (
    <div className="flex min-h-[80vh] flex-col items-center justify-center text-center">
      <div className="relative mb-12 flex items-center justify-center">
        ...
        <div className="absolute size-64 animate-pulse-ring rounded-full border border-accent/10" style={{ animationDelay: "1s" }} />
      ...
      <h1 className="text-3xl font-extralight leading-snug tracking-tight">衝動に、<br />振り回されない。</h1>
      <p className="mt-6 text-sm font-light leading-relaxed text-muted-foreground">その90秒が、未来を変える。</p>
      ...
      <p className="mt-6 text-[10px] tracking-widest text-muted-foreground">/ 我慢ではなく、観察のアプリです</p>
    </div>
  );
}
...
function BreathSuccess({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    const i = setInterval(() => {
      ...
      setSec(rem);
      if (rem <= 0) { clearInterval(i); setDone(true); }
    }, 100);
  ...
  if (done) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center text-center">
        ...
        <h2 className="mt-3 text-2xl font-extralight tracking-tight">もう、始まっています。</h2>
        <p className="mt-4 max-w-[26ch] text-sm font-light leading-relaxed text-muted-foreground">
          いま呼吸できた、それがあなたの最初の1回。次に衝動がきたら、同じことをするだけです。
        </p>
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
      <div className="relative flex items-center justify-center">
        <div className="absolute size-48 rounded-full bg-accent/20 blur-2xl transition-transform duration-1000 ease-in-out" style={{ transform: `scale(${scale})` }} />
        <div className="absolute size-36 rounded-full border border-accent/40 bg-accent/5 transition-transform duration-1000 ease-in-out" style={{ transform: `scale(${scale})` }} />
        <div className="relative flex flex-col items-center">
          <span className="text-6xl font-extralight tabular-nums tracking-tighter">{Math.ceil(sec)}</span>
          <span className="mt-1 text-[10px] tracking-[0.3em] text-accent">{label}</span>
        </div>
      </div>
      <p className="mt-12 text-xs font-light text-muted-foreground">/ 急がず、合わせなくていい</p>
